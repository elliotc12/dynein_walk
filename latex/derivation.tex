...
We want to find cartesian coordinates in terms of $L_n$ and $L_f$.\\
...
\begin{align}
  \dot{X}_{nm} =
  &-\frac{\text{Ls} \sqrt{1-\frac{\left(\text{Ln}(t)^2+\text{Ls}^2-\text{Lt}^2\right)^2}{4
   \text{Ls}^2 \text{Ln}(t)^2}} \left(\frac{\text{Ln}'(t)
   \left(L^2-\text{Lf}(t)^2+\text{Ln}(t)^2\right)^2}{2 L^2
   \text{Ln}(t)^3}-\frac{\left(L^2-\text{Lf}(t)^2+\text{Ln}(t)^2\right) \left(2
   \text{Ln}(t) \text{Ln}'(t)-2 \text{Lf}(t) \text{Lf}'(t)\right)}{2 L^2
   \text{Ln}(t)^2}\right)}{2
   \sqrt{1-\frac{\left(L^2-\text{Lf}(t)^2+\text{Ln}(t)^2\right)^2}{4 L^2
   \text{Ln}(t)^2}}} - \\
  &\frac{\text{Ln}'(t) \left(L^2-\text{Lf}(t)^2+\text{Ln}(t)^2\right)
   \left(\text{Ln}(t)^2+\text{Ls}^2-\text{Lt}^2\right)}{2 L \text{Ls}
   \text{Ln}(t)^3}\\
  &-\frac{\text{Ls}
   \sqrt{1-\frac{\left(L^2-\text{Lf}(t)^2+\text{Ln}(t)^2\right)^2}{4 L^2 \text{Ln}(t)^2}}
   \left(\frac{\text{Ln}'(t) \left(\text{Ln}(t)^2+\text{Ls}^2-\text{Lt}^2\right)^2}{2
   \text{Ls}^2 \text{Ln}(t)^3}-\frac{\text{Ln}'(t)
   \left(\text{Ln}(t)^2+\text{Ls}^2-\text{Lt}^2\right)}{\text{Ls}^2 \text{Ln}(t)}\right)}{2
   \sqrt{1-\frac{\left(\text{Ln}(t)^2+\text{Ls}^2-\text{Lt}^2\right)^2}{4 \text{Ls}^2
       \text{Ln}(t)^2}}}\\
  & +\frac{\text{Ln}'(t) \left(L^2-\text{Lf}(t)^2+\text{Ln}(t)^2\right)}{2
   L \text{Ls} \text{Ln}(t)}+\frac{\left(2 \text{Ln}(t) \text{Ln}'(t)-2 \text{Lf}(t)
   \text{Lf}'(t)\right) \left(\text{Ln}(t)^2+\text{Ls}^2-\text{Lt}^2\right)}{4 L \text{Ls}
   \text{Ln}(t)^2}
\end{align}

This is becoming really ugly! I think I want to try another thing. Tomorrow I'll try and implement a different angle scheme and see if it works out easier...this is not tenable. The advantage to this scheme is that it avoids ambiguity, but it is also really really complicated. And even this model has ambiguity in that certain angles could have positive/negative values, so the C code will have to have memory and guess which value is correct. So I think it would be good to choose a simpler ambiguous model and code in ambiguity-resolution to the C code.\\
...
\begin{align}
  Y_{nb} &= 0 \\
  Y_{nm} &=  \\
  Y_{t}  &= L_n\sqrt{1 - \big(\frac{(L^2+L_n^2-L_f^2}{2LL_n}\big)^2} \\
  Y_{fm} &=  \\
  Y_{fb} &= 0
\end{align}
...
\begin{align}
  \dot{X}_{nb} &= 0 \\
  \dot{X}_{nm} &= \\
  \dot{X}_{t } &= \frac{L^2+2L_n\dot{L}_n-2L_f\dot{L}_f}{2L} \\
  \dot{X}_{fm} &= \\
  \dot{X}_{fb} &= 0
\end{align}  
...
\begin{align}                                                                          
  \dot{Y}_{nb} &= 0 \\                                         
  \dot{Y}_{nm} &= \\
  \dot{Y}_{t}  &= \frac{L_n}{2\sqrt{1 - \big(\frac{(L^2+L_n^2-L_f^2}{2LL_n}\big)^2}} * \big(\frac{(-L^2L_n^{-2}+\dot{L}_n - 2L_f\dot{L}_nL_n^{-1} + L_f^2L_n^{-2}}{2L}\big) - think this is rite?\\
  \dot{Y}_{fm} &= \\
  \dot{Y}_{fb} &= 0 \\
\end{align}
