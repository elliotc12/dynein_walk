...
\begin{center}
  \begin{tabular}{| l | l | l | l | l | l |}
    ...
    $\theta_{b}$ & 56 & & & &\\ \hline
    $\theta_{t}$ & 0 & & & &\\ \hline
    $k_{ub}$ & $180 s^{-1,a}$? review & & & &\\ \hline
    $k_b$ & $1000 s^{-1,b}$? review & & & &\\ \hline
  \end{tabular}
\end{center}

a: The transition from post-stroke to prestroke in the Winch paper is given by three transitions: the binding of ATP ($k_{+ATP} = 2.2*10^{6} M^{-1}s^{-1}$), dissociation from the microtubule ($k_{-MT} = 460 s^{-1}$) and ATP hydrolysis/taking on the prestroke conformation ($k_{RS} = 180 s^{-1}$). We take the rate-limiting step $k_{RS}$ as our transition rate from bothbound to onebound.\\

b: The transition from prestroke to poststroke in the Winch paper is given by three transitions: binding to the microtubule ($k_{+MT} = 10^4 s^{-1}$), releasing a phosphate from hydrolysis ($k_{-Pi} = 5000 s^{-1}$), the power stroke itself ($k_{PS} = 5000 s^{-1}$) and ADP release ($k_{-ADP} = 1000 s^{-1}$).\\

\subsection{Stalk and tail length}
Sarlah et. al use stalk and tail lengths of 12 and 14 nm, respectively. We used PyMol to examine the crystal structures 3VKH and XXXX (pre and post-powerstroke) to find
these values.
...
These figures show different measurements for equilibrium angles from the data in this paper:\\

\begin{figure}[h!]
  \centering
  \begin{minipage}[b]{0.45\textwidth}
    \includegraphics[width=\textwidth]{../figures/Post_powerstroke_tomography.jpg}
    \caption{Post-powerstroke equilibrium angles from EM tomography.}
  \end{minipage}
  \hspace{5mm}
  \begin{minipage}[b]{0.45\textwidth}
    \includegraphics[width=\textwidth]{../figures/Pre_powerstroke_tomography.jpg}
    \caption{Pre-powerstroke equilibrium angles from EM tomography.}
  \end{minipage}
