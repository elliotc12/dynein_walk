...
\begin{align}
  F{P(t_0+1,x')} &= F{\int_{-\infty}^{\infty}W(x'-x)P(t_0,x)dx}\\
  \tilde{P(t_0+1,k)} &= N\tilde{W(k)}\tilde{P(t_0,k)}\\
\end{align}
...
\begin{align}
  \tilde{P(n,k)} &= N\tilde{W(k)}^n\tilde{P(t_0,k)}\\
\end{align}

We then take the Inverse Fourier Transform of this expression, yielding:

\begin{align}
  P(n,x) &= \int_{-\infty}^{\infty}\tilde{P(n,k)}e^{2\pi ikx}dk
  = N\int_{-\infty}^{\infty}\tilde{W(k)}^n\tilde{P(t_0,k)}e^{2\pi ikx}dk\\
\end{align}

Since $P(0,x)$ is a delta function, we know that $\tilde{P}(0,x) = 1$, so:

\begin{align}
  P(n,x) &=  N\int_{-\infty}^{\infty}\tilde{W(k)}^ne^{2\pi ikx}dk\\
\end{align}
