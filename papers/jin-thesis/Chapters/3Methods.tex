...
Despite simulating many combinations of parameters, the number of free variables still produce many degrees of freedom. This makes it difficult to test the entire parameter space and truly invesitage the relationships between different sets of parameters. We approach this problem by individually attributing the parameters to a specific data set from exerpiment. For instance, the exponential unbinding constant, $C$, will be fitted specifically to Figure (\ref{fig:trailingbias}). We use time data (\textit{cite paper for time data}\cite{}) to fit the unbinding rate constant, $k_{ub}$, since the units are $s^{-1}$ and the inverse will yield the average time it takes to unbind, i.e. the both bound time. The rebinding rates, $k_p$ and $k_b$, can also be fitted to experiment's one bound time, but these rates are directly correlated to dynein's step length. The ability for dynein to take long steps are dependent on how long dynein diffuses above the microtubule. Therefore, the rebinding rates will have a much larger parameter space due to the flexibility of optimizing according to the one bound time or step length. Since we are interested in Yildiz's step lengths, we will match these rates to the linear regression from Figure (\ref{fig:YildizCorrelation}). The last set of parameters are the spring constants. The spring constants also influence the step length because they dictate the strength of the restoring force. We fit the spring constants last to control the variance of step lengths.
...
\end{equation}
Now, the final step is to solve for the probability distribution of distances, $p(L_i)$. Since we want the probability of absolute differences, we can neglect the distinction between `initial' and `final' and calculate an equilibrium distribution of distances, $p(L)$. To do so, we used a reoccurence relation, where we defined an initial $p^0(L)$ that is not in equilibrium and multiplied it by the conditional probability distribution for arbitrary $L$'s, $p(L'|L)$, until it reaches equilibrium. That is,
\begin{align*}
	p^1(L)= & p(L'|L)p^0(L) \\
