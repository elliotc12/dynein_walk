...
\matmethods{
  ...
  where $F^{x/y}_i$ is the force projection on coordinate $x/y$ of domain $i$, and $R^x_i$ the Brownian force. Transforming position variables to polar coordinates results in four degrees of freedom: $\theta_{bb}, \theta_{bm}, \theta_{um} and \theta_{ub}$. Performing this transformation and expanding the force term yields the following equations:

  \begin{multline}
    \dot{X_i}\left(\theta_1, ..., \theta_n, \dot{\theta_1}, ..., \dot{\theta_n}\right) = \frac{1}{\gamma_i}\big(F^{spr}_x(\theta_1, ..., \theta_n) + \\
    \lambda_i\left(X_{i+1}-X_i\right) + \lambda_{i-1}\left(X_i-X_{i-1}\right)R^x_i\big)
    \label{eq:ob-system}
  \end{multline}

  \begin{multline}
    \dot{Y_i}\left(\theta_1, ..., \theta_n, \dot{\theta_1}, ..., \dot{\theta_n}\right) = \frac{1}{\gamma_i}\big(F^{spr}_y(\theta_1, ..., \theta_n) + \\
