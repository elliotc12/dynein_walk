...
\title{Brownian dynamics simulation of cytoplasmic dynein reproduces bidirectional stepping}
...
--Background on dynein
--Evidence for dynein's powerstroke

However, to our knowledge, no work has been done to study dynein dynamics computationally. Existing models use chemical rate transitions and assume independence of steps \cite{sarlahmodel}. No models exist which account for the diffusion dynamics or conformational changes of dynein as it walks along the microtubule. Understanding the coordinated dynamics of dynein's domains is necessary to fully describe the protein's motility. Testing whether the current powerstroke model is capable of generating processive motion would be a good first step to understanding dynein's dynamics.\\

Here we present a course-grained model of cytoplasmic dynein which treats its domains as connected rigid bodies. Chemical transitions and conformational changes consistent with the powerstroke model are accounted for via elastic forces. The dynamics are determined using Brownian dynamics. The model is capable of generating directed motility at rates similar to experiment. Further, we show that the model displays further complex behavior, such as tension gating and force-mediated bidirectionality.\\
...
\begin{figure}[tbhp]
  \centering
  a)\includegraphics[width=\linewidth]{../../plots/paper_displacement_vs_step_length.pdf}
  b)\includegraphics[width=\linewidth]{../../plots/paper_onebound_vs_steplength.pdf}
  c)\includegraphics[width=\linewidth]{../../plots/paper_initial_vs_final_displacement.pdf}
