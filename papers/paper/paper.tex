...
\matmethods{
  ...
  \section*{Onebound motion equations}
  ...
  where $F^{x/y}_i$ is the force projection on coordinate $x/y$ of domain $i$, and $R^x_i$ the Brownian force. Expanding the force term and transforming position variables to polar coordinates yields the following:

  \begin{multline}
    \dot{X_i}\left(\theta_1, ..., \theta_n, \dot{\theta_1}, ..., \dot{\theta_n}\right) = \frac{1}{\gamma_i}\big(F_x(\theta_1, ..., \theta_n) + \\
    \lambda_i\left(X_{i+1}-X_i\right) + \lambda_{i-1}\left(X_i-X_{i-1}\right)R^x_i\big)
    \label{eq:ob-system}
  \end{multline}

  \begin{multline}
    \dot{Y_i}\left(\theta_1, ..., \theta_n, \dot{\theta_1}, ..., \dot{\theta_n}\right) = \frac{1}{\gamma_i}\big(F_y(\theta_1, ..., \theta_n) + \\
    \lambda_i\left(Y_{i+1}-Y_i\right) + \lambda_{i-1}\left(Y_i-Y_{i-1}\right)R^y_i\big)
    \label{eq:ob-system}
  \end{multline}

  where the $\lambda$ terms are tension terms needed to maintain the rigid rod constraints. Each $X_i$ coordinate is related to the $\theta_j$ coordinates via the recursive formulas $X_1 = L_1\cos(\theta_1)$ and $X_{i>1} = L_i\cos(\theta_i)+X_{i-1}$ for x-coordinates and $Y_1 = 0$ and $Y_{i>1} = L_i\sin(\theta_i)+Y_{i-1}$ for y-coords. Thus, each $\dot{X_i}$ term is linearly related to one or more $\dot{\theta_j}$ terms.\\
