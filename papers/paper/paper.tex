...
\subsection*{$k_{ub}$}
The unbinding rate $k_{ub}$ was estimated from stepping trace data in the Yildiz \textit{et. al.} experiment. The $k_{ub}$ value which led to an average velocity of 26 nm/s was chosen, as shown in Figure \ref{fig:TrajPlot}.
...
\subsection*{$k_b$, $k_{a}$}
The binding and affinity rates directly influenced the onebond stepping statistics shown in Figure \ref{fig:SteppingPlots}. The binding rate controlled the tendency for immediate steps. The affinity rate controlled the transition time from low affinity to high affinity and introduced a buffer time after unbinding to prevent instantaneous steps. This was crucial when fitting to Yildiz's slope of m = -0.4, as our model was only able to replicate the stepping correlation if dynein prefered steps within 8 nm.  


\begin{figure}[tbhp]
\centering
\includegraphics[width=1\linewidth]{../../plots/mc_plots/u_step_length_probability_distribution_2.0_5.50e+10_1.00e+08_0.0_1.0_1.0_120.0_197.0_242.0_-0.5}
\includegraphics[width=1\linewidth]{../../plots/mc_plots/u_step_length_1d_probability_density_2.0_5.50e+10_1.00e+08_0.0_1.0_1.0_120.0_197.0_242.0_-0.5}
