...
\title{Simulating dynein's powerstroke using Brownian dynamics}
...
%% \title{Simulating a Brownian model of the dynein motor protein}
%% \author{E. L.}{Capek}
...
%% \dedication{Yet to come.}

%% \acknowledgements{Yet to come.}
...
\textbf{Abstract}
\blockquote{Dynein is a motor protein which uses cellular energy to transport cargo around the cell. Like related motor proteins kinesin and myosin, dynein uses ATP bond breaking to alternatingly kick its two foot domains forward and generate motion. Unlike the more regularly-stepping kinesin or myosin, dynein's stepping pattern is very random: it can take steps between zero and 60\textit{nm} in both the forwards and backwards directions. It is believed that dynein takes broad, stochastic steps because it is large and has several elastic regions, making it more affected by Brownian motion. To test this, we model the motor as a 2D system of springy hinges, then simulate this model using Brownian dynamics. Preliminary results indicate such a model is capable of taking steps between 0 and 25nm. These results give hope that, with further tweaking, the model may be able to generate both larger and backwards steps.}
...
\subsubsection{Why is dynein interesting?}
...
\subsection{Validating model}
The simulation was first validated for obeying the laws of physics, and in particular, the equipartition theorem. Figure \ref{fig:equipartition_agreement} shows how both models have their angular potential energy converge to the expected $\frac{n}{2}k_BT$ energy over time.\\
...
\subsection{Model achieves processivity}
At certain input parameters, the model can successfully take steps. Figure \ref{fig:stepping-ghosts} shows snapshots of the model at various times throughout a simulation. The model steps back and forth, taking both forwards and backwards steps.\\

\begin{figure}[h]
  \centering
  \begin{tabular}{| l | l | l | l | l |}
    \hline
    $k_{b}$ & $k_{ub}$ & $c_b$ &  $c_m$ & $c_t$\\\hline
    $10^{18} s^{-1}$ & $10^{20} s^{-1}$ & $2.0\Delta G_{ATP}$ & $2.0\Delta G_{ATP}$ & $1.0\Delta G_{ATP}$\\ \hline
  \end{tabular}
  \caption{Parameters used in simulations.}
  \label{table:thesis-parameters}
\end{figure}

More detailed stepping information is shown in Figure \ref{fig:stepping-x-trajectory}.a. The model steps both forwards and backwards, but has a definite forward bias. The model's behavior can be contrasted with experimental behavior in Figure \ref{fig:stepping-trajectory}.b. The two trajectory plots look very different, but share major key features: forwards and backwards steps, a directional bias, and varying step sizes. At a minimum, the two-state stepping model 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{../../plots/x-trajectory_thesis}
  \includegraphics[width=0.5\textwidth]{../../plots/y-trajectory_thesis}
  \caption{Near and far binding domains over the course of a simulation. Simulation run at standard conditions with $k_b=123 s^{-1}, k_{ub}=456 s^{-1}, c_b=c_m=c_t=789 \Delta G_{ATP}$.}
  \label{fig:stepping-x-trajectory}
\end{figure}

\subsection{Model steps reasonably}
...
\subsection{Model does not drag as it steps}
The model lifts its foot up and lowers it as it is about to step; it doesn't drag along the microtubule searching for the binding site. This behavior is shown in Figure \ref{fig:stepping-y-trajectory}.\\

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{../../plots/y-trajectory_thesis}
  \caption{Unbound binding domain position over the course of a simulation. Simulation run at standard conditions with $k_b=123 s^{-1}, k_{ub}=456 s^{-1}, c_b=c_m=c_t=789 \Delta G_{ATP}$.}
  \label{fig:stepping-y-trajectory}
\end{figure}
...
\subsection{Current work: Refining model parameters}
