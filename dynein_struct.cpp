...
void Dynein::update_brownian_forces() {
  if (brownian_testcase) {
    ...
    rand.gauss2(sqrt(2*kb*T/(gb*dt)), &r.blx, &r.bly);
    rand.gauss2(sqrt(2*kb*T/(gm*dt)), &r.mlx, &r.mly);
    rand.gauss2(sqrt(2*kb*T/(gt*dt)), &r.tx, &r.ty);
    rand.gauss2(sqrt(2*kb*T/(gm*dt)), &r.mrx, &r.mry);
    rand.gauss2(sqrt(2*kb*T/(gb*dt)), &r.brx, &r.bry);
...
void Dynein::update_internal_forces() {
  if (internal_testcase) {
    ...
    f.blx = 0;     f.bly = 0;   // Initialize forces to zero
    f.mlx = 0;     f.mly = 0;
    f.tx  = 0;     f.ty  = 0;
    f.mrx = 0;     f.mry = 0;
    f.brx = 0;     f.bry = 0;
...
double Dynein::get_mlx() {
  return ls * cos(get_bla()) + blx;
}

double Dynein::get_mly(){
  return ls * sin(get_bla()) + bly;
}

double Dynein::get_tx() {
  return ls * cos(get_bla()) + lt * cos(get_mla()) + blx;
}

double Dynein::get_ty(){
  return ls * sin(get_bla()) + lt * sin(get_mla()) + bly;
}
...
double Dynein::get_d_mlx() {
  return ls * d_bla * -sin(bla);
}

double Dynein::get_d_tx() {
  return lt * d_mla * -sin(mla) + get_d_mlx();
}

double Dynein::get_d_mrx() {
  return lt * d_mra * sin(mra) + get_d_tx();
}

double Dynein::get_d_brx() {
  return ls * d_bra * sin(bra) + get_d_mrx();
}
...
double Dynein::get_d_mly() {
  return ls * d_bla * cos(bla);
}

double Dynein::get_d_ty() {
  return lt * d_mla * cos(mla) + get_d_mly();
}

double Dynein::get_d_mry() {
  return lt * d_mra * -cos(mra) + get_d_ty();
}

double Dynein::get_d_bry() {
  return ls * d_bra * -cos(bra) + get_d_mry();
}
